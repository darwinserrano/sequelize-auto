...
AutoSequelize.prototype.build = function (callback) {
  ...
  function mapTable(table, _callback) {
    var schema = null;
    if (self.sequelize.options.dialect === 'postgres' || self.sequelize.options.dialect === 'postgres-native'){
      schema = self.options.schema;
    }

    if (schema) {
      self.queryInterface.describeTable(table, schema).then(setTableDescription, _callback);
    } else {
      self.queryInterface.describeTable(table).then(setTableDescription, _callback);
    }

    function setTableDescription (fields) {
  ...
  this.queryInterface.showAllTables(this.options).then(function (__tables) {
...
AutoSequelize.prototype.run = function (callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function (table, _callback) {
      ...
      text[table] += spaces+ "const "+table+" =  sequelize.define('" + tableName + "', {\n";
